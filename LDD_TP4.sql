...
BEGIN
  FOR l_societe IN (SELECT nomSociete, numSociete
                    FROM Societe) LOOP
...
SELECT SUM(Achat.nbArticles * (Achat.prixAchat - Article.prixGrossiste))
...
BEGIN
  ...
  DBMS_OUTPUT.PUT_LINE(l_numclient || ' a acheté ' || l_nbAchat || ' produits.');
END;
/
...
SET SERVEROUTPUT ON
DECLARE
  l_nomclient Client.nomClient%TYPE := UPPER('&ask');
  l_nbAchat NUMBER;
BEGIN
  SELECT COUNT(*)
  INTO l_nbAchat
  FROM Achat INNER JOIN Client ON (Achat.numClient = Client.numClient)
  WHERE nomClient = l_nomclient;
  
  DBMS_OUTPUT.PUT_LINE(l_nomclient || ' a acheté ' || l_nbAchat || ' produits.');
END;
/
...
SET SERVEROUTPUT ON
BEGIN
  FOR l_client IN (SELECT nomClient, numClient
                    FROM CLient) LOOP
                    
    DBMS_OUTPUT.PUT_LINE('Liste des achats de ' || l_client.nomClient);
    
    FOR l_societe IN (SELECT DISTINCT Societe.nomSociete, Societe.numSociete
                        FROM Achat INNER JOIN Societe ON (Achat.numSociete = Societe.numSociete) INNER JOIN Client ON (Achat.numClient = Client.numClient)
                        WHERE Client.nomClient = l_client.nomClient) LOOP
                        
      DBMS_OUTPUT.PUT('    - à ' || l_societe.nomSociete || ' : ');
      
      FOR l_achat IN (SELECT Article.nomArticle
                      FROM Article INNER JOIN Achat ON (Article.numArticle = Achat.numArticle) INNER JOIN Societe ON (Achat.numSociete = Societe.numSociete)
                      WHERE (Achat.numClient = l_client.numClient) AND (Achat.numSociete = l_societe.numSociete)) LOOP
                      
        DBMS_OUTPUT.PUT(l_achat.nomArticle || ',');
